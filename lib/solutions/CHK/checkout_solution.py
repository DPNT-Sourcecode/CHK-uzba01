...
def checkout(skus):
    ...
    stxyz_count = sum(skus_to_counts[sku] for sku in "STXYZ")
    total_price = calculate_total_price(
        sku_count=stxyz_count,
        price=skus_to_base_prices["S"],  # Assuming all have the same price
        discounts=group_discount_for_s_t_x_y_z
    )

    # Subtract the counted STXYZ from the total counts
    for sku in "STXYZ":
        skus_to_counts[sku] = 0
